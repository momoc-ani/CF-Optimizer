...
func TestClearDiscoveredDomainsRemovesAutomaticAccelerationAndPreservesManualDomain(t *testing.T) {
	...
	if result.Cleared != 2 || result.AccelerationsRemoved != 1 || !result.PolicyRefreshed {
		t.Fatalf("unexpected cleanup result: %#v", result)
	}
	state := stateStore.Snapshot()
	if len(state.DiscoveredDomains) != 0 {
		t.Fatalf("discovered records remain: %#v", state.DiscoveredDomains)
	}
