...
| 阶段10：已验证域名加速 | 已完成（Linux/macOS 真机待发布） | 精确加速域名与 Mihomo 连接自动发现；物理接口 DNS 解析与 Cloudflare 网段/SNI 预检；Mihomo `hosts`、`DOMAIN,DIRECT`、优选 IP `DIRECT` 热加载；sing-box/Xray DNS 映射、External JSON-RPC、无代理 Hosts 映射；跨节点切换、订阅刷新、失败和卸载的累计收据与幂等回滚；设置与代理页面展示发现和验证证据 | Windows 真机已验证 `ani.momoc.top` HTTPS 远端地址等于优选 IP，连接经指定物理接口与网关，Mihomo 活动连接命中 `DIRECT`；Go、Vitest、Playwright 与六目标交叉编译通过，Linux/macOS 真实代理组合和系统域名映射仍待对应真机发布验收 |
