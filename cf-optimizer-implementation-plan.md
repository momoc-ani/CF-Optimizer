...
“每6小时重新优选”表示节点池刷新周期，而不是每次点击都重复完整测速。一次一键优选仍由后台自动完成，但内部按检查点执行以下阶段：

1. `POOL_REFRESH`：按官方网段生成候选，执行 TCP、TLS 和下载复筛，成功后原子替换版本化节点池。
2. `POOL_REUSE`：有效节点池只做当前物理路径的轻量 TCP/TLS 复核；节点池过期时先尝试刷新，刷新失败则复核旧池并继续使用，旧池不会被失败刷新覆盖。
3. `DOMAIN_QUALIFY`：消费节点池完成物理 DNS、Cloudflare CIDR、SNI/Host 预检和手动域名同域下载复测。每个手动域名按节点排名尝试，首个达到阈值立即选用，不继续寻找更快地址。
4. `POLICY_PLAN`：根据节点池和域名证据生成策略计划，不修改系统。
5. `APPLY_VERIFY`：应用过渡/最终策略，验证真实连接、远端 IP 和代理配置；失败只重试应用阶段并按收据回滚本轮变更。
6. `COMMIT`：原子提交节点、域名映射、策略收据和历史；提交失败不会把成功节点池清除。

节点池和 `OptimizationCheckpoint` 写入版本化状态文件，检查点只在节点池 ID、测速配置哈希、物理路径指纹一致时复用；配置、网段或网络路径发生变化会使相关证据失效。后台状态轮询只返回节点池摘要和当前阶段，不暴露完整候选与策略收据。
...
- `config.get` 返回已原子保存的期望配置；手动域名和排除域名在单任务锁内刷新当前策略并热更新，其他字段由 `config.update.restart_required` 明确标记为后台服务重启后生效。
- `acceleration.clear_discovered` 仅清理自动发现记录；若这些域名已有映射，服务先应用并验证仅保留手动域名的新策略，再原子删除发现记录，任一步失败都保留原记录和已验证策略。
